...
/**
 * POST /api/exams/<examSlug>/compose
 * examSlug: nclex | naplex | pance | aanp-fnp | npte-pt | usmle-step-1|2|3
 * Body: { numQuestions, focusAreas?, difficultyPreference?, outputFormat?, seed? }
 * Returns a curated + anti-cluster-sequenced exam with selection/sequencing analytics.
 */
export async function POST(
  ...
  if (!config) {
    return NextResponse.json(
      {
        error: `Unknown exam "${examSlug}".`,
        supported: listComposableExamSlugs(),
      },
      { status: 404 }
    );
  }
  ...
  const difficultyRaw = (body.difficultyPreference ?? body.difficulty_preference) as
    | string
    | undefined;
  ...
  try {
    const exam = await composePracticeExam(config.slug, {
    ...
    console.error(`[api/exams/${config.slug}/compose] failed:`, error);
    return NextResponse.json(
      { error: `Failed to compose ${config.examName} exam.` },
      { status: 500 }
    );
  }
}
