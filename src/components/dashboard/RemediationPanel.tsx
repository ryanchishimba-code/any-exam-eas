    ...
    <section aria-labelledby="remediation-heading" className={`${dbUi.heroSurface} space-y-4`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          >
            {open === 0 ? "No open loops" : `${open} missed item${open === 1 ? "" : "s"} still open`}
          </h2>
          <p className={`${dbUi.subtitle} mt-2`}>
            {open === 0
              ? `Nothing on ${examName} is waiting on a later correct attempt.`
              : `Missed ${examName} topics that are still outstanding.`}
          </p>
      ...
      <details className="rounded-2xl border border-[var(--color-border)]/50 bg-[var(--color-surface)]/50 px-3.5 py-2.5">
        ...
        <p className="mt-2 pb-1 text-[11px] leading-relaxed text-[var(--color-ink-muted)]">
          ...
          one. An item leaves this list after a later correct attempt on that item.
        </p>
      </details>
