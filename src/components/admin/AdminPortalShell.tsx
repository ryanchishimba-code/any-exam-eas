  ...
  const sidebar = (
    <aside className="flex h-full w-60 shrink-0 flex-col border-r border-slate-200 bg-white p-4">
      ...
      <div className="mt-auto space-y-2 border-t border-slate-200 pt-4">
        <Link
          href="/internal"
          className="block px-3 py-2 text-xs text-slate-500 hover:text-slate-800"
        >
          Employee portal →
        </Link>
        <button
          ...
          className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900 disabled:opacity-60"
        >
          <LogOut size={16} />
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto flex min-h-screen max-w-7xl">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {mobileOpen && (
            <div className="border-b border-slate-200 bg-white p-4 md:hidden">{sidebar}</div>
          )}
