...
export function scoreExamQuality(
  ...
  const perQuestion = exam.questions.map((q) => {
    ...
    return scoreQuestionHeuristic(q, subjectModule);
  });
  const average =
...
function scoreQuestionHeuristic(q: ExamQuestion, subjectModule: SubjectModule): number {
  let score = 0.55;
  ...
  if (q.explanation.length > 80) score += 0.08;
  else if (q.explanation.length > 40) score += 0.04;

  const distractorCount = Object.keys(q.distractorRationale ?? {}).length;
  if (distractorCount >= 3) score += 0.1;
  else if (distractorCount >= 2) score += 0.06;

  if (q.clinicalReasoning && q.clinicalReasoning.length > 40) score += 0.06;

  if (q.references?.length) score += 0.04;

  if (q.correctAnswer && q.options?.includes(q.correctAnswer)) score += 0.05;

  if (q.highYield) score += 0.03;
