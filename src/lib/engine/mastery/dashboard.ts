...
type MasteryDashboardPayload = {
  rollup: MasteryRollup;
  mapTiles: DomainMapTile[];
};

async function cachedMasteryDashboard(
  userId: string,
  examKey: string,
  loader: () => Promise<MasteryDashboardPayload | null>
): Promise<MasteryDashboardPayload | null> {
  return cacheGetOrSet(
    cacheKey(["mastery-dashboard", userId, examKey]),
...
export async function loadNclexMasteryDashboard(userId: string): Promise<{
  ...
  return cachedMasteryDashboard(userId, "nclex", async () => {
...
export async function loadNaplexMasteryDashboard(userId: string): Promise<{
  ...
  return cachedMasteryDashboard(userId, "naplex", async () => {
...
export async function loadUsmleMasteryDashboard(
  ...
  return cachedMasteryDashboard(userId, `usmle:${step}`, async () => {
