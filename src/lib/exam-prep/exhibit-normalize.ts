/**
 * Normalize AI exhibit / lab payloads into renderable stem tables.
 * Shared by USMLE and NCLEX.
 */
import type { BankItem } from "@/lib/question-bank";
import type { ExhibitFigureRef } from "./exhibit-figure";
...
/** Build a canonical exhibit table from ngnPayload / chartData shapes. */
...
/**
 * Write renderable `table` onto ngnPayload.
 * @param promoteExhibitKind — USMLE may promote vignette→exhibit; NCLEX NGN must keep kind.
 */
export function normalizeExhibitTablePayload(
  item: BankItem,
  opts?: { promoteExhibitKind?: boolean }
): BankItem {
  const promote = opts?.promoteExhibitKind ?? false;
  ...
  const kind = typeof ngn.kind === "string" ? ngn.kind : "";
  const isNgn = NGN_KINDS.has(kind) || NGN_KINDS.has(item.itemType ?? "");

  if (promote && !isNgn && (!ngn.kind || ngn.kind === "vignette" || ngn.kind === "mcq")) {
