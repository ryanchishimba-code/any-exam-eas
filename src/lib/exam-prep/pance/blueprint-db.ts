/**
 * PANCE blueprint reference data — DB-backed categories, topics, and question helpers.
 */
...
export type PanceBlueprintCategoryRow = {
  slug: string;
  kind: "content" | "task";
  label: string;
  weight: number;
...
export async function loadPanceBlueprintCategories(): Promise<PanceBlueprintCategoryRow[]> {
  try {
    const rows = await prisma.panceBlueprintCategory.findMany({
...
export async function loadPanceTopics(subjectId?: string): Promise<PanceTopicRow[]> {
  try {
    const rows = await prisma.panceTopic.findMany({
...
function fallbackBlueprintCategories(): PanceBlueprintCategoryRow[] {
  ...
  const tasks = PANCE_TASK_CATEGORIES.map((t, i) => ({
    slug: t.id,
    kind: "task" as const,
    label: t.label,
    weight: t.weight,
...
export async function resolvePanceDeepDiveSlug(params: {
  subjectId?: string;
  blueprintTopic?: string;
  reviewModuleSlug?: string;
}): Promise<string | undefined> {
  if (params.reviewModuleSlug) return params.reviewModuleSlug;
  const topics = await loadPanceTopics(params.subjectId);
