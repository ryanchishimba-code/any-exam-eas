/**
 * Insert PANCE bank items with content-hash dedup and connection retries.
 */
...
async function withDbRetry<T>(label: string, fn: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 1; attempt <= MAX_DB_RETRIES; attempt++) {
    try {
      ...
      console.warn(`[pance] ${label} DB retry ${attempt}/${MAX_DB_RETRIES} in ${waitMs}ms…`);
      await sleep(waitMs);
    }
  }
  throw lastError;
...
export type PanceInsertResult = {
  created: number;
  skipped: number;
};

export async function insertPanceBankItems(
  prisma: PrismaClient,
  items: BankItem[]
): Promise<PanceInsertResult> {
  let created = 0;
  let skipped = 0;
