#!/usr/bin/env node
/**
 * Keep only the top-quality NCLEX items (default 7,000) and retire the rest.
 *
 ...
 *
 *   npm run db:trim-nclex-to-target:dry
 *   npm run db:trim-nclex-to-target
 *   npm run db:trim-nclex-to-target -- --target 7000
 */
...
import { assessNclexItemQuality, type NclexQualityVerdict } from "../src/lib/exam-prep/nclex-quality-gate";
import { NCLEX_TARGET_TOTAL } from "../src/lib/exam-prep/nclex/types";
import { enrichBankItemFromRow } from "../src/lib/mpje/parse-bank-options";

const prisma = new PrismaClient();
const BATCH = 500;
const ROOT = process.cwd();
const LOG = join(ROOT, "artifacts", "trim-nclex-to-target.json");

type RankedRow = {
  id: string;
  subjectId: string;
  tier: NclexQualityVerdict["tier"];
  score: number;
  issueCount: number;
  rank: number;
  source: string | null;
...
function tierWeight(tier: NclexQualityVerdict["tier"]): number {
...
function computeRank(verdict: NclexQualityVerdict, tags: string | null, source: string | null): number {
...
async function main() {
  ...
  console.log(`\nNCLEX trim — keep top ${target} by quality (${total} active now)${dryRun ? " [dry-run]" : ""}\n`);
  ...
  if (!dryRun) {
    ...
    while (true) {
      const rows = await prisma.questionBankItem.findMany({
        where: { fieldId: "nursing", active: true, ...(retireCursor ? { id: { gt: retireCursor } } : {}) },
        orderBy: { id: "asc" },
        take: BATCH,
        select: { id: true },
      });
  ...
  if (eligible < target) {
    ...
    console.log(`\n✓ NCLEX bank capped at ${keep.length} serve-ready items.`);
  }
}
