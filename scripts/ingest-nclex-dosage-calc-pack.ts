#!/usr/bin/env node
/**
 * Ingest community NCLEX dosage calculation pack (100 items) → AI elevate → best QA gate.
 *
 * Usage:
 *   npm run db:ingest-nclex-dosage-calc-pack
 *   npm run db:ingest-nclex-dosage-calc-pack -- --dry-run
 *   npm run db:ingest-nclex-dosage-calc-pack -- --skip-curate
 */
...
const PACK_TAG = "community-dosage-calc-100";
...
async function main() {
  ...
  console.log(
    `\nNCLEX dosage calc pack — ${COMMUNITY_NCLEX_DOSAGE_CALC_100.length} items${dryRun ? " [dry-run]" : ""}${curateOnly ? " [curate-only]" : ""}\n`
  );
  ...
  if (curateOnly) {
    ...
    for (const row of COMMUNITY_NCLEX_DOSAGE_CALC_100) {
      const item = communityDosageItemToBankItem(row);
      ...
      if (dryRun) {
        console.log(`  [dry-run] #${row.id} would insert: ${row.question.slice(0, 60)}…`);
        continue;
      }
