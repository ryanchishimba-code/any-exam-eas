#!/usr/bin/env node
/**
 * Attach approved educational figures to AANP FNP bank items.
 * Purpose-keyword gated; normalizes exhibit findings/labTable → tables.
 *
 * Usage:
 *   npm run db:attach-aanp-fnp-figures -- --dry-run --limit 100
...
import type { AanpFnpFigureRef } from "../src/lib/exam-prep/aanp-fnp/figure-assets";
import { attachVisualRationaleToItem } from "../src/lib/engine/rationale/enrich-visual-rationale";

const prisma = new PrismaClient();
const BATCH = 150;
const AANP_FIELDS = ["aanp-fnp"] as const;
...
function hasApprovedMedia(ngn: Record<string, unknown> | undefined): boolean {
  ...
  return (ngn.media as AanpFnpFigureRef[]).some((m) => m?.reviewStatus === "approved");
}
...
async function main() {
  ...
  console.log(
    `AANP FNP figure attach${dryRun ? " [dry-run]" : ""} field=aanp-fnp${
