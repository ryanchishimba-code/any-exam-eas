#!/usr/bin/env node
/**
 * Attach approved educational figures to NAPLEX (pharmacy) bank items.
 * Purpose-keyword gated; normalizes exhibit findings/labTable → tables.
 * Preserves item kinds (constructed, SATA, drag_drop, exhibit, …).
 *
 * Usage:
 *   npm run db:attach-naplex-figures -- --dry-run --limit 100
...
import type { NaplexFigureRef } from "../src/lib/exam-prep/naplex/figure-assets";
import { attachVisualRationaleToItem } from "../src/lib/engine/rationale/enrich-visual-rationale";

const prisma = new PrismaClient();
const BATCH = 150;
const NAPLEX_FIELDS = ["pharmacy"] as const;
...
function hasApprovedMedia(ngn: Record<string, unknown> | undefined): boolean {
  ...
  return (ngn.media as NaplexFigureRef[]).some((m) => m?.reviewStatus === "approved");
}
...
async function main() {
  ...
  console.log(
    `NAPLEX figure attach${dryRun ? " [dry-run]" : ""} field=pharmacy${
